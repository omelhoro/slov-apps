...
for l in excs:
    for tsk in l[1:]:
        task_obj={}
        task_obj["les"]=l[0][0].replace("les","")
        expl_topic=tsk[0].split("|")
        assert len(expl_topic)==2
        topic_grammar_pos=expl_topic[0].split(",")
        assert len(topic_grammar_pos)==2
        task_obj["pos"]=topic_grammar_pos[0]
        task_obj["grammar"]=topic_grammar_pos[1]
        task_obj["expl"]=expl_topic[1]
        task_obj["task"]=make_input(tsk[1])
        assert all(isinstance(itm, str) for itm in task_obj.viewvalues())
        newexcs.append({k:v.strip() for k,v in task_obj.viewitems()})

print(newexcs)
    
ex= {e[0][0].strip():{ky[0].strip():make_input(ky[1]) 
                      for ky in e[1:]} 
     for e in excs }
...
js=json.dumps(newexcs,indent=1)
with open("../excsApp.json","w+") as f:
    f.write(js)
